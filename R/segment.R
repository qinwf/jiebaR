#' @export
jiebar <- function(type = "mix", dict = "inst/dict/jieba.dict.utf8", hmm = "inst/dict/hmm_model.utf8", 
    user = "inst/dict/user.dict.utf8", idf = "inst/dict/idf.utf8", stop_word = "inst/dict/stop_words.utf8", 
    qmax = 20, topn = 5) {
    switch(type, 
...
#' @export
seg <- function(code, jiebar, symbol = F, lines = 1e+05, qmax = 20, output = paste0("rjieba", 
    as.numeric(Sys.time()), ".dat"), encoding = "UTF-8") {
    if (!is.character(code) || length(code) != 1) 
        stop("Argument 'code' must be an string.")
    if (file.exists(code)) {
        FILESMODE <- T
        cutl(code = code, jiebar = jiebar, symbol = symbol, lines = lines, 
            qmax = qmax, output = output, encoding = encoding, FILESMODE = FILESMODE)
    } else {
        FILESMODE <- F
        cutw(code = code, jiebar = jiebar, qmax = qmax, symbol = symbol, 
            FILESMODE = FILESMODE)
    }
}

cutl <- function(code, jiebar, symbol, lines, qmax, output, encoding, FILESMODE) {
    nlines <- lines
    ...
    tryCatch({
        while (nlines == lines) {
            ...
            if (nlines > 0) {
                ...
                out.lines <- cutw(code = tmp.lines, jiebar = jiebar, qmax = qmax, 
